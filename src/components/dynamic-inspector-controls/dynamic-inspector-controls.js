...
const DynamicInspectorControls = ( {
	...
	const availableTaxonomiesWithTerms = useSelect(
		( select ) => {
			...
			const taxTree = availableTaxonomies.map( ( tax ) => {
				const terms =
					getEntityRecords( 'taxonomy', tax.slug, {
						per_page: -1,
						hide_empty: true,
					} )?.map( ( term ) => ( {
						...term,
				...
				let selectedTermIds =
					attributes.taxonomyTerms.find(
						( taxonomyTerm ) => taxonomyTerm.slug === tax.slug
					)?.terms || [];

				// Handle legacy attributes from before multiple taxonomy support
				if ( attributes.taxonomy && terms ) {
					selectedTermIds = attributes.terms.filter(
						( selectedTermId ) =>
	...
	const onTermsChange = ( taxonomySlug, termNames ) => {
		const taxonomyWithTerms = availableTaxonomiesWithTerms.find(
			( taxonomyWithTerm ) => taxonomyWithTerm.slug === taxonomySlug
		);
		const termIds = taxonomyWithTerms.terms
			.filter( ( term ) => termNames.includes( term.name ) )
			.map( ( term ) => term.id );

		let taxonomyTerms = attributes.taxonomyTerms.filter(
			( taxonomyTerm ) => taxonomyTerm.slug !== taxonomyWithTerms.slug
		);
		taxonomyTerms = [
			...taxonomyTerms,
			{
	...
	const authorsSuggestions = availableAuthors?.map(
		( author ) => author.name
	);
	...
	const onAuthorsChange = ( authorNames ) => {
		const authorIds = availableAuthors
	...
	const onPostTypeCheckboxChange = ( postTypeSlug ) => {
		...
		const newSelectedPostTypes = postTypeWasSelected
			? selectedPostTypes.filter(
					( _postTypeSlug ) => _postTypeSlug !== postTypeSlug
			  )
			: [ ...selectedPostTypes, postTypeSlug ];
	...
	const selectSearchResult = ( searchResult ) => {
		setAttributes( {
			handpickedPosts: [
				...attributes.handpickedPosts,
				{ position: 1, post: searchResult },
			],
		} );
	};

	const existingPosts = attributes.handpickedPosts.map(
		( existing ) => existing.post
	);

	const removeHandpickedPost = ( existingPostId ) => {
		setAttributes( {
			handpickedPosts: attributes.handpickedPosts.filter(
				( existing ) => {
	...
	const setPosition = ( existingPostId, position ) => {
		const existingIndex = attributes.handpickedPosts.findIndex(
			( existing ) => existing.post.id === existingPostId
		);
		setAttributes( {
			handpickedPosts: [
	...
	return (
		<InspectorControls>
					...
					{ availablePostTypes.map( ( postType ) => {
						return (
							<CheckboxControl
								key={ postType.slug }
								onChange={ () =>
									onPostTypeCheckboxChange( postType.slug )
								}
								label={ postType.name }
								checked={ attributes.postType
									.split( ',' )
			...
			availableAuthors?.length ? (
				...
				>
					{ availableTaxonomiesWithTerms.map(
						( taxonomyWithTerms ) => {
							return (
								<FormTokenField
									...
									suggestions={
										taxonomyWithTerms.suggestions
									}
									onChange={ ( taxonomyTermNames ) =>
										onTermsChange(
											taxonomyWithTerms.slug,
											taxonomyTermNames
										)
									}
			...
			>
				...
				/>
				{ [ 'meta_value', 'meta_value_num' ].includes(
					attributes.orderby
				) ? (
					<TextControl
					...
					{ attributes.handpickedPosts.length ? (
						<div className="splx-handpickedPostsWrap">
							...
							<div className="splx-handpickedPosts">
								{ attributes.handpickedPosts.map(
									( handpicked ) => {
										return (
