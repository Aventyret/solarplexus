...
<div class="has-global-padding is-layout-constrained wp-block-query alignwide">
	<ul class="is-layout-flow is-flex-container columns-<?php echo esc_attr(
 	$columns
 ); ?> alignwide splx-wp-block-post-template">
		<?php foreach ($args['posts'] as $post):
  	setup_postdata($post); ?>
		<li class="wp-block-post post-<?php the_ID(); ?> post type-<?php echo esc_attr(
 	get_post_type()
 ); ?> status-publish<?php if (
 	has_post_thumbnail()
