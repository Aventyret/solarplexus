...
<div class="has-global-padding is-layout-constrained wp-block-query alignwide">
	<ul class="is-layout-flow is-flex-container columns-<?php echo $columns; ?> alignwide splx-wp-block-post-template">
   	...
   	setup_postdata($post); ?>
		<li class="wp-block-post post-<?php the_ID(); ?> post type-<?php echo get_post_type(); ?> status-publish<?php if(has_post_thumbnail()): ?> has-post-thumbnail<?php endif; ?>">
			...
			<?php if (array_key_exists('showExcerpt', $args['config'])): ?><p class="wp-block-post-excerpt"><?php echo get_the_excerpt() ?></p><?php endif; ?>
			<?php if (array_key_exists('showDate', $args['config'])): ?><div class="wp-block-post-date"><time datetime="<?php the_date('c'); ?>"><a href="<?php the_permalink(); ?>"><?php echo get_the_date(); ?></a></time></div><?php endif; ?>
